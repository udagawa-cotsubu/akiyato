...
import type { WeeklyAdrPoint } from "@/lib/lodging/metrics";

interface WeeklyAdrChartProps {
  data: WeeklyAdrPoint[];
  weeks: string[];
}
...
/** 縦軸用: 大きな数は万円表記で短く（例: 50000 → ¥5万、12345 → ¥1.2万） */
...
function WeeklyAdrTooltip({ active, payload, label }: CustomTooltipProps) {
  if (!active || !payload || payload.length === 0) return null;
  const p = payload[0];
  const innId = p.dataKey as string;
  const innName = (p.payload?.[`${innId}_label`] as string | undefined) ?? "宿";
  const adr = p.value as number;
  const totalSale = p.payload?.[`${innId}_totalSale`] as number | undefined;
  const totalNights = p.payload?.[`${innId}_totalNights`] as number | undefined;
  ...
  return (
    <div className="rounded-md border bg-card px-3 py-2 text-xs shadow-md">
      ...
      <div className="mt-1 space-y-0.5">
        {totalSale != null && <div>販売金額合計: {formatYen(totalSale)}</div>}
        {totalNights != null && <div>宿泊数: {totalNights}泊</div>}
        <div>1泊あたりの単価: {formatYen(adr)}</div>
      </div>
    </div>
  );
}
...
/** 宿毎の週次ADR（販売金額合計÷宿泊数＝1泊あたりの単価）を表示する棒グラフチャート */
export function WeeklyAdrChart({ data, weeks }: WeeklyAdrChartProps) {
  ...
  const chartData = weeks.map((weekKey) => {
    ...
    for (const innId of innIds) {
      ...
      if (point) {
        row[innId] = Math.round(point.adr);
        row[`${innId}_label`] = point.innName ?? "宿";
        row[`${innId}_totalSale`] = point.totalSaleAmount;
        row[`${innId}_totalNights`] = point.totalNights;
      } else {
        row[innId] = 0;
        row[`${innId}_label`] = innNameById.get(innId) ?? "宿";
        row[`${innId}_totalSale`] = 0;
        row[`${innId}_totalNights`] = 0;
      }
    }
    return row;
  });
  ...
  return (
    <ResponsiveContainer width="100%" height={320}>
      <BarChart data={chartData} margin={{ top: 4, right: 8, bottom: 2, left: 24 }}>
        ...
        <Tooltip content={<WeeklyAdrTooltip />} />
        <Legend />
        {bars}
      </BarChart>
    </ResponsiveContainer>
