...
export interface YearCompareAdrRow {
  weekLabel: string;
  "2024": number;
  "2025": number;
  "2026": number;
...
/** 1宿分の週次ADRから年比較用の行データを生成（1W〜53W、2024/2025/2026を横並び） */
export function buildAdrYearCompareData(innData: WeeklyAdrPoint[]): YearCompareAdrRow[] {
  const map = new Map<string, number>();
  innData.forEach((p) => {
    const match = /^([0-9]{4})\s+([0-9]+)W$/.exec(p.weekKey);
    if (!match) return;
    const [, year, week] = match;
    map.set(`${year}_${week}`, Math.round(p.adr));
  });
  const rows: YearCompareAdrRow[] = [];
...
interface WeeklyAdrYearCompareChartProps {
  data: YearCompareAdrRow[];
  innName: string;
}
...
export function WeeklyAdrYearCompareChart({
  data,
  innName,
}: WeeklyAdrYearCompareChartProps) {
  return (
